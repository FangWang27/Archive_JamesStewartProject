...
perfect_match_trials_july5th <- function(
mu0 = 0, 
mu1 = 1,
num.real.dmr = 10,
delta = 1,
...
vec.delta1[region.dmr] <- delta + runif(sum(region.dmr),0,0.5)
...
vec.mu1[region.dmr] <- runif(sum(region.dmr),-0.5,0.5) + mu1
...
toc()
toc()
return( list( alt.tabs = alt.tabs, real.index = real.index, sample.used <- alt.samples))
}
