dmrIdentifier_opt <- function(methyl.MVal, cluster, patient.num = 30, B = 250,k = 5,site.info) {
  # The function obtained by modifying author's code
  # since the running-median smoothing has the window size of k, we may remove the cluster and corresponding
  #sites that are samaller than k. This can be done since the cluster are generate in the way that the size is increasing.
  # than k. The value k = 5 is set by the author

  chr <- rep(1, length(cluster))
  require(bumphunter)
  ...
  stat <- bumphunter::runmedByCluster(y = rawStat.wt.sign, x = NULL, cluster = cluster, weights = NULL, k = k, endrule = "constant", verbose = FALSE)
  ...
  tab <- bumphunter::regionFinder(x = stat, chr = chr, pos = pos.gen, cluster = cluster, cutoff = cutoff, ind = Index, verbose = FALSE)
  
  
  ##### Calculate p-values for candidate DMRs
  if (nrow(tab) > 0) {
    nulltabs <- lapply(1:B, function(i) {
      bumphunter::regionFinder(x = permStat[, i], chr = chr, pos = pos.gen, cluster = cluster, cutoff = cutoff, ind = Index, verbose = FALSE)
    })
