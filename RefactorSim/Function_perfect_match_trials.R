...
# the null samples generated, where each column coutains all the reading values of a single tissue. For any tissue
# corresponding to the i th column, the (i + num.pairs ) column contains the reading that is paired to it.
null.samples <- (mu0 + t(mvsample.Ns)) * mat.z
...
for (index in 1:sample.N) {
  ...
  null.tabs[[index]] <- dmrIdentifier_opt(null.sample, clusters, sites, num.pairs, B, k = 5, cluster.lengths)
  alt.tabs[[index]] <- dmrIdentifier_opt(alt.sample, clusters, sites, num.pairs, B, k = 5, cluster.lengths)
}
toc()
toc()
return( list( null.tabs = null.tabs, alt.tabs = alt.tabs, real.index = real.index))
}
